<template>
    <div class="post">
        <div class="actions">
            <img :src="'/icons/more.svg'" alt="" @click="moreButtonClicked">
            <div class="editPostAction" :class="isMoreButtonActive ? 'active' : ''">
                <router-link :to="{name: 'post.edit'}"><img :src="'/icons/pen.svg'" alt="" :class="isMoreButtonActive ? 'enabled' : 'disabled'" @click="editPostClicked"></router-link>
            </div>
            <div class="deletePostAction" :class="isMoreButtonActive ? 'active' : ''">
                <div class="agreement" :class="isDeleteButtonClicked ? 'active' : ''">
                    <p id="delete" @click="postDelete">Delete</p>
                    <p id="cancel" @click="isDeleteButtonClicked = !isDeleteButtonClicked">Cancel</p>
                </div>
                <img :src="'/icons/trash-can.svg'" alt="" :class="isMoreButtonActive ? 'enabled' : 'disabled'" @click="isDeleteButtonClicked = !isDeleteButtonClicked">
            </div>
        </div>
...
.post .actions img.disabled {
    opacity: 0;
    height: 0;
    width: 0;
}

.post .actions img.enabled {
    opacity: 1;
    width: 20px;
    height: 20px;
}
...
.deletePostAction.active {
    height: 20px;
    margin-bottom: 10px;
}

.deletePostAction .agreement {
    display: flex;
    gap: 1rem;
    opacity: 0;
    margin-right: 0;
...
.deletePostAction .agreement.active {
    opacity: 1;
    margin-right: 1rem;
}

.deletePostAction .agreement p {
    visibility: hidden;
    transition-duration: .2s;
    cursor: pointer;
    margin: 0;
...
.deletePostAction .agreement.active p {
    visibility: visible;
}
